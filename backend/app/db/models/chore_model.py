...
class Chore(Base):
    ...
    title = Column(String, nullable=False, index=True) # Index title for searching
    ...
    family = relationship("Family", back_populates="chores") # Assumes 'chores' relationship on Family model

    # The user currently assigned to do this chore instance (can be null if unassigned)
    assignee_id = Column(PG_UUID(as_uuid=True), ForeignKey("user.id"), nullable=True, index=True)
    assignee = relationship("User", foreign_keys=[assignee_id], backref="assigned_chores")

    # The user who originally created this chore template/schedule
    creator_id = Column(PG_UUID(as_uuid=True), ForeignKey("user.id"), nullable=False)
    creator = relationship("User", foreign_keys=[creator_id], backref="created_chores")
    ...
    recurrence = Column(SQLEnum(ChoreRecurrence, name="chore_recurrence_enum", create_type=True), nullable=False, default=ChoreRecurrence.ONCE)
    ...
    status = Column(SQLEnum(ChoreStatus, name="chore_status_enum", create_type=True), nullable=False, default=ChoreStatus.PENDING, index=True)
    ...
    verified_by_id = Column(PG_UUID(as_uuid=True), ForeignKey("user.id"), nullable=True)
    verified_by = relationship("User", foreign_keys=[verified_by_id], backref="verified_chores")
    ...
    reward_delivered = Column(Boolean, default=False, nullable=False)
    ...
    tags = Column(JSONB, nullable=True) # Example: ["kitchen", "cleaning", "allowance"]
